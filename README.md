...
<details>
<summary><strong>왜 이 repo가 필요한가</strong> — NCT로 서울 in-region에 갇히면 frontier의 ~37%, 이 repo는 self-host로 ~82%까지 끌어올린다 (펼쳐서 논증·벤치마크 보기)</summary>
...
**메시지.** *"NCT 때문에 서울에 갇혀 frontier의 37%짜리 모델밖에 못 쓴다"* 는 통념을, 이 **2-command CDK 솔루션**(`cdk deploy --all` → `finalize-deploy.sh`)이 *"frontier 대비 80% 수준을 서울 in-region으로 유지하면서 NCT를 AWS에서 준수한다"* 로 바꾼다. frontier 100%는 아니지만 80% 수준이면 대부분의 실무를 커버하며, 무엇보다 **데이터 주권을 깨지 않는다.**

> 수치 근거·해석 주의·더 높은 충실도(Qwen3.5-397B 등) 옵션은 아래 **Model Matrix**의 *성능은 얼마나 떨어지는가* 펼침 섹션 참고. 벤치마크는 harness·설정에 따라 ±편차가 있으니 **도입 전 실제 워크로드로 PoC 실측** 권장.
...
vLLM pod가 모델 weight를 받을 때 HF 토큰을 쓴다. **없으면 vLLM pod가 기동에 실패하고(GPU 노드는 점유한 채 과금) `NctEksStack` 배포가 즉시 멈춘다**(deploy-time sanity check가 `hf-token` 부재를 잡는다).
...
기본 6종 중 **2종이 gated** = HF에서 라이선스 동의가 선행돼야 다운로드된다. 승인 없이 배포하면 해당 vLLM pod가 **HF 401/403**으로 기동에 실패한다.
...
- **반드시 위에서 토큰을 발급한 동일 HF 계정으로** 각 모델카드의 "Agree and access"(access request)를 먼저 눌러 승인받을 것.
- 나머지 4종(`coding`/`video`=Qwen3.5-27B[Apache-2.0], `ocr`=InternVL3-14B, `audio`=Phi-4 Multimodal)은 gated 아님 — 추가 승인 불요.
- **두 모델을 안 쓸 거면** 해당 vLLM 스택(`NctVllm-Longcontext`·`NctVllm-Math`)을 배포에서 빼면 된다(아래 *단계별 배포*에서 제외, 또는 `cdk deploy`에서 해당 스택 생략). gated 승인 없이도 나머지 스택은 정상 배포된다.
- ⚠️ gated 여부·라이선스 조건은 모델카드 정책이라 바뀔 수 있다. **배포 시점에 각 모델카드에서 확인**할 것.
...
- **gated 모델**(`math`·`longcontext`)은 HF 라이선스 승인이 선행돼야 한다 — [Prerequisites](#prerequisites) 2번 참고. 안 쓰면 해당 스택을 배포에서 빼면 된다.
...
- ⚠️ 라이선스·벤치마크는 모델카드 기준이며 변할 수 있다 — **배포 시점에 각 모델카드에서 확인**.
...
> 모든 가격은 작성 시점 개략 추정치다. 최신 요율은 [AWS Pricing Calculator](https://calculator.aws/)에서 확인할 것.
